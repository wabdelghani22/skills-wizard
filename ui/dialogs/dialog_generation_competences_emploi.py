...
class DialogGenerationCompetencesParEmploi(QDialog):
    def __init__(self, titre_fichier, contenu_fichier, emplois_existants, suggestions_ia=None,
                 ...
                 nb_niveaux=5, competences_detectees=None, parent=None, source=""):
        ...
        self.competences = []
        self.competences_detectees = competences_detectees
        self.source = source
        ...
        self.layout.addWidget(QLabel(f"<b>Source :</b> {self.source}"))

        if self.contenu_fichier or self.suggestions:
            ...
            familles = list({emp.get("famille", "") for emp in self.emplois_existants})
            sous_familles = list({emp.get("sous_famille", "") for emp in self.emplois_existants})

            self.input_titre = QLineEdit()
            self.input_famille = QComboBox()
            self.input_famille.setEditable(True)
            self.input_famille.addItems(sorted(set(familles)))
            self.input_sous_famille = QComboBox()
            self.input_sous_famille.setEditable(True)
            self.input_sous_famille.addItems(sorted(set(sous_familles)))
        ...
        self.btn_cancel = QPushButton("Passer")
        self.btn_ok.clicked.connect(self.accept)
        self.btn_cancel.clicked.connect(self.reject)
        btns.addStretch()
        btns.addWidget(self.btn_ok)
        btns.addWidget(self.btn_cancel)
        self.btn_arreter = QtWidgets.QPushButton("⛔ Arrêter")
        self.btn_arreter.clicked.connect(self.demander_arret)
        btns.addWidget(self.btn_arreter)
        # Ajoute le bouton dans le layout (où tu veux)
        #self.layout().addWidget(self.btn_arreter)
    ...
    def generer_competences(self):
        ...
        try:
            prompt = self.prompt_builder(emploi, self.contenu_fichier, params, self.competences_detectees)
    ...
    def get_nom_emploi(self):
        if not self.contenu_fichier and not self.suggestions:
    ...
    def get_famille_sous_famille_si_nouveau(self):
        if self.combo_emploi.currentText() == "Créer un nouvel emploi...":
            return (
                self.input_titre.text().strip(),
                self.input_famille.currentText().strip(),
                self.input_sous_famille.currentText().strip()
            )
        return (None, None, None)
    def demander_arret(self):
        self.arret_demande = True
        #self.reject()  # Ferme le dialog proprement sans valider
        self.parent().interruption_generation = True
        self.reject()
